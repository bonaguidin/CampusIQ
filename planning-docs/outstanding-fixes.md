...
### Workday source — wired in 2026-08-29 (branch `feat/workday-ingest`), open follow-ups

`scripts/job_postings/workday.py` is now wired into the ingest via `run_workday()` (`--source workday`) and a parallel `workday-ingest` job in `postings-ingest.yml`. Scoped to the **12** of 19 `ats=workday` employers in `data/job_postings/dfw_employers_ats.csv` that have a buildable slug today. Staged migration `20260829120000_job_postings_target_role_nullable.sql` relaxes `job_postings.target_role` for whole-board rows — **still needs a manual `supabase migration up` after review**, same as its two `job_postings` siblings.
...
- [ ] **`identity.py` has no Workday host pattern, so cross-source exact dedup can't fire for Workday rows.** `_ATS_URL_PATTERNS` is host-keyed for Greenhouse/Lever/Ashby/SmartRecruiters; Workday needs a regex-host branch (tenants are `<tenant>.wd<N>.myworkdayjobs.com`, plus the shared `wd<N>.myworkdaysite.com` host) **and** a path-tail requisition-number extractor that reproduces `workday._job_id()`'s `bulletFields[0]`-preferred value from the URL alone. Deferred from the wiring PR: it's more than a regex addition, and a wrong extraction heuristic silently forks or merges clusters (the exact failure `identity.py`'s own comments warn about). Until then Workday rows cluster by fuzzy employer+title only — correct, just weaker against a syndicated Adzuna/JSearch copy of the same posting. Only one real Workday `externalPath` sample exists (`/job/Texas---Dallas/Sr-Applications-Developer_JR13846`); collect a few more slug formats first.
...
- [ ] **`missing_grade_scale` finding needs a home outside "Still needs your review."** It's one of `NON_BLOCKING_WARNING_CODES` (`reconciliation.py`) so it never blocks `calculator_ready`, and there is no correction that clears it — but it still renders in the review list (`GradeCalculatorPanel.tsx` `<GeneralFindings>`, copy "This syllabus doesn't specify a letter-grade scale."), framed as something to fix. Unlike `unknown_assessment_count` (moot since PR #64 and now filtered out of the list), `missing_grade_scale` is genuinely meaningful: with no letter scale the calculator can't produce a projected *letter* grade or resolve a letter target (the numeric calc still works). It belongs as a soft note near the target-grade section, not in the blocking review list. Deliberately left visible for now rather than silently filtered.
