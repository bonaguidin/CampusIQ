  ...
  return (
    <form className="profile-completion-form" onSubmit={(event) => { void submit(event); }} noValidate>
      ...
      <section className="profile-form-section">
        ...
        <label className={`${fieldClass('student.major_intended')} profile-form-field--gated`}>Intended major<input value={majorIntended} onChange={(e) => setMajorIntended(e.target.value)} disabled={!switching} placeholder={switching ? 'e.g. Computer Science' : ''} aria-describedby="intended-major-help" /><small id="intended-major-help">{helper('student.major_intended') ?? (switching ? 'The major you are switching to.' : 'Not switching — we will record this as N/A.')}</small></label>
