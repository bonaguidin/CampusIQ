...
For the 10 uncovered SOCs today, GAP still emits confident must-have/nice-to-have splits and is asked to cite an O\*NET importance score ([line 101](../../GradusIQ_career/gradus_iq_prompt_GAP.md#L101)) it was never given.
...
**Two fields get dropped:** `role_family` and `typical_education` are hand-curated, not derivable from O\*NET, and are never passed to the model — [market_data.py:153-163](../../GradusIQ_career/features/market_data.py#L153) forwards only title, job_zone, and the three requirement lists. Safe to drop; note it so nobody looks for them later.
...
**Supabase is deferred** (consistent with the dashboard plan's own Phase 1/Phase 2 split). Student data is already Postgres-backed via [supabase_client.py](../../GradusIQ_career/supabase_client.py), but reference data is file-backed and there's no reason to move static reference data behind a network call for this milestone. The loader's CSV output is unaffected, so a Supabase import remains available later.
...
[market_data.py:146](../../GradusIQ_career/features/market_data.py#L146) sets `matched = bool(soc and entry)`. Every occupation now has an entry, so `matched` is `True` for all 14 demo roles — including Finance Intern and Operations Intern, which return empty requirement lists. The `notes` list, which previously warned "mapped to SOC but no O\*NET entry exists", is now empty.
...
[role_research_agent.py:59](../../GradusIQ_career/features/role_research_agent.py#L59) reads this same file and tags results `agent_onet_corroborated` when the SOC appears in it. Its own comment says the catalog is *"only 10 occupations — far too narrow to use for rejection."*
...
**File:** [GradusIQ_career/features/market_data.py](../../GradusIQ_career/features/market_data.py)
...
**Files:** [gap.py](../../GradusIQ_career/features/gap.py), [gradus_iq_prompt_GAP.md](../../GradusIQ_career/gradus_iq_prompt_GAP.md)

Today [gap.py:142](../../GradusIQ_career/features/gap.py#L142) calls `role_research_agent.get_role_requirements(role)` for **every** role on every run, and the prompt then forbids using its skills output for scoring. The agent's research is paid for and discarded.
...
**Files:** [shift.py](../../GradusIQ_career/features/shift.py), [gradus_iq_prompt_SHIFT.md](../../GradusIQ_career/gradus_iq_prompt_SHIFT.md), [role_research_agent.py](../../GradusIQ_career/features/role_research_agent.py)
...
Both prompt files use `{{target_roles}}`, `{{expected_graduation}}`, `{{ai_anxiety_level}}` and document that they "are interpolated from the student JSON." No interpolation exists — [base.build_messages()](../../GradusIQ_career/features/base.py#L93) concatenates the template, the contract, and a student-context JSON block. There is no `.format`, template engine, or substitution anywhere in `GradusIQ_career`.
...
**The code is done; the demo does not reflect it yet.** `data/demo_cache/` was built 2026-08-02, before any of Steps A–E. The dashboard serves those files directly ([api.py](../../GradusIQ_career/api.py) is cache-first and returns before any live call), so **every feature the demo shows is pre-fix output.**
...
```bash
uv run python -m GradusIQ_career.demo.build_demo_cache --mock --only priyaNair && git checkout data/demo_cache/
```
...
Separate from the rebuild: a local checkout needs `GRADUSIQ_PROXY_SECRET` set or every API route returns 503 (`authorize_proxy_request` fails closed), and `npm install --prefix frontend`. Neither is related to this branch — but the rebuild can't be eyeballed in the UI until they're done.
